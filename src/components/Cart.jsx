...
const handlePurchase = async () => {
  ...
  // Map cartItems to only include the necessary data
  const purchaseItems = cartItems.map(item => ({
    proizvod_id: item.id, // Change to match the database field for the product ID
    kolicina: item.quantity || 1 // Make sure you have a quantity, default to 1 if necessary
  }));
  
  console.log("Purchase Items:", purchaseItems);
  
  if (korisnikId && purchaseItems.length > 0) {
    await zavrsiKupovinu(korisnikId, purchaseItems);
    dispatch(setClearCartItems()); 
  } else {
