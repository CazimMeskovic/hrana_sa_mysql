...
const Cart = () => {
  ...
  // Function to handle the "send to basa" button click
  const onSendToBase = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/cart', {
        ...
        body: JSON.stringify({ cartItems, totalAmount, totalQTY }),
      });

      if (response.ok) {
        const data = await response.json();
        console.log('Cart items sent successfully:', data);
        // Optionally clear the cart after successful submission
        dispatch(setClearCartItems());
      } else {
        console.error('Failed to send cart items');
      }
    } catch (error) {
              ...
              <div className="fixed bottom-0 bg-white w-full px-5 py-2 grid items-center">
                ...
                <div className="grid items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={onSendToBase} // Call the send function on click
                  >
                    Send to Base
                  </button>
                </div>
